...
class _CallOutScreenState extends State<CallOutScreen> {
  ...
  void loadTimerState() async {
    ...
    setState(() {
      ...
      lastEmailSentAt = prefs.getInt('lastEmailSentAt_${widget.callOutId}') ?? 0;
    });
  ...
  Future<void> _loadBreakStatus() async {
    ...
    setState(() {
      onBreak = prefs.getBool('${widget.callOutId}_${widget.EmployeeId}_onBreak') ?? false;
    });
  }
                      ...
                      try {
                        ...
                        if (calloutSnapshot.exists) {
                          ...
                          if (empIndex != -1) {
                            ...
                            pauseTimer();
                            Workmanager().cancelByUniqueName("timerTask_${widget.callOutId}");
                            SharedPreferences prefs = await SharedPreferences.getInstance();
                            await prefs.remove('elapsedSeconds_${widget.callOutId}');
                            await prefs.remove('onBreak_${widget.callOutId}');
                            await prefs.remove('lastEmailSentAt_${widget.callOutId}');
                            setState(() {
